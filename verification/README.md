...
这个目录保存 Zhulong 的可复跑验证。`baselines/` 保存可长期引用的稳定摘要；`reports/` 保存本地或当次 CI 生成的临时明细，默认不进入 Git；Playwright 截图写入 `.zl-tmp/visual/`。验证用于证明 Zhulong 当前使用 `zl-*` 作为主动 workflow 命令面，并把 GraphRAG 和 Graphify 作为增强后端接入。
...
npm run verify:project-profiles
...
`verify:project-profiles` 使用两套真实 fixture 执行相同的 codebase、graph、workflow、evidence 和 completion 主链：非文档密集型项目使用 `reference + rag none` 并断言无 RAG 副作用；文档密集型项目使用 `strict + rag local` mock 并验证索引、查询和 answer audit。
...
`verify:pages` 组装 GitHub Pages 允许列表，只发布文档、cockpit 样例、稳定验证基线和许可证，并阻止临时报告、截图、源码、runtime、脚本、依赖目录或符号链接进入站点。`verify:public-release` 检查 Apache-2.0、npm 公开元数据、Pages 渲染链接、社区健康文件、Dependabot、安全报告入口和常见密钥模式。
