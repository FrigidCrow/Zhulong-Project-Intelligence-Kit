...
本次增强把 Zhulong 从“命令和 workflow 能跑”推进到“RAG、citation、trace、policy、help skills 和完整命令面都有可复跑证据”。

核心目标：

1. 增加 MVP3 Evidence Quality & Policy Mode。
2. 增加 `zl-help-skills`，根据用户场景推荐合适的 `zl-*` 命令组。
3. 更新 README 和所有文档入口。
4. 增加 changelog，能追溯每个阶段做了什么。
5. 增加完整测试计划和两轮正式测试报告入口。
6. 用脚本证明 `package.json` 中所有 `zl-*` / `zl` 命令都能实际执行。

## 2. 新增命令

| 命令 | 作用 | 主要产物 |
| --- | --- | --- |
| `zl-rag-golden-add` | 增加 RAG golden case | `.planning/quality/rag-goldens.json` |
| `zl-rag-golden-run` | 运行 RAG golden cases | `.planning/quality/RAG_GOLDEN_RESULTS.md` |
| `zl-rag-eval` | 汇总 RAG 可信度 | `.planning/quality/RAG_EVAL.md` |
| `zl-citation-audit` | 校验 citation 指向真实源文件 | `.planning/knowledge/CITATION_AUDIT.md` |
| `zl-trace-build` | 建立文档、代码、测试、evidence trace matrix | `.planning/trace/TRACE_MATRIX.json` |
| `zl-trace-query` | 查询 trace matrix | stdout |
| `zl-trace-audit` | 审计 trace matrix 完整性 | `.planning/trace/TRACE_AUDIT.md` |
| `zl-policy-list` | 列出内置 policy | `.planning/policies/POLICY_LIST.md` |
| `zl-policy-check` | 执行 policy checks | `.planning/policies/POLICY_CHECK.md` |
| `zl-policy-explain` | 解释指定 policy | stdout |
| `zl-help-skills` | 根据场景推荐命令 | `.planning/help/HELP_SKILLS.md` |
...
| `scripts/verify-full-command-surface.mjs` | `npm run verify:full-command-surface` | `package.json` 中全部 `zl-*` / `zl` bin 命令 |
...
1. `zl-docs-sync`：文档更新后一键 scan / extract / diff / index / citation audit。
2. `zl-answer-audit`：检查回答是否有 citation、是否命中规格、是否有幻觉风险。
