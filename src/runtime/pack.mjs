...
function runtimeLocalContract(values) {
  return `\n\n## Zhulong Local Runtime Contract\n\n- Use the local Zhulong CLI: \`${values.ZL_CLI}\`.\n- Read and follow \`${values.ZL_KIT_ROOT}/core/workflows/authorization.md\` before acting.\n- A suggested next \`zl-*\` command is never authorization to execute it. Without explicit user intent or a matching bounded-autonomy Goal, run only the current skill and return control to the user.\n- Workflow commands do not infer who invoked them. When this skill is the direct response to the current user message, the runtime must append \`--source user-message\`; for an agent-selected downstream skill it must not. This is a runtime assertion, not cryptographic identity proof. It authorizes the requested work, not acceptance of its result. Append \`--accept-completion\` only if the same user message explicitly asked to complete or close it.\n- Goal-driven child workflows never use \`--source user-message\`. They must use the exact objective from a structured milestone contract and pass \`--authorization\`, \`--milestone\`, and \`--contract-digest\`.\n- Before adding dependencies or running commit, push, merge, or release operations under a Goal, require \`zl workflow permission-check --target <repo> --permission <name>\` to pass.\n- Investigation, analysis, and debugging default to diagnose-only. Modify code only for an explicit fix/implementation request or an in-scope Goal grant.\n- When the user explicitly requests automatic execution of named milestones, translate that natural-language request into a project-local contract file and one bounded authorization.\n- Never use self-authored evidence as user acceptance. Stop for unresolved material decisions and at the authorization boundary.\n- Keep project data local-only by default. Do not route document content to external providers unless the project explicitly opts in.\n- Public workflow commands may run lightweight status checks, but they must not trigger hidden heavy refresh. GraphRAG index, Graphify build, and refresh-run require explicit \`--run\`, \`--index\`, or \`zl-refresh-run\` approval.\n- Preserve evidence writeback: record meaningful verification with \`zl-evidence-record\` and use \`--writeback\` when closing workflow work.\n`;
}
