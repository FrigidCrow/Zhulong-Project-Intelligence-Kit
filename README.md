...
- 仕様書、QA、議事録、设计文档新增或更新后，重跑 `pik-docs-scan --target "$PWD"`、`pik-docs-extract --target "$PWD"` 和 `pik-docs-diff --target "$PWD"`。
...
| 文档/RAG | `pik-docs-scan`, `pik-docs-status`, `pik-docs-normalize`, `pik-docs-extract`, `pik-docs-diff`, `pik-docs-citations`, `pik-citation-audit`, `pik-rag-init-local`, `pik-rag-golden-add`, `pik-rag-golden-run`, `pik-rag-eval`, `pik-docs-index`, `pik-docs-query` |
