...
description: Zhulong local project cockpit for Codex. Use when the user asks for a project cockpit, leader demo, visual report, project health view, or a Graphify/RAG/evidence status summary.
---

# Zhulong Project Cockpit

This is the Codex runtime entrypoint for `$zl-cockpit-build`.

## Invocation

- Treat `$zl-cockpit-build` as the project cockpit / visualization command.
- Use it when the user asks for 驾驶舱, leader 演示, 可视化报告, 项目健康度, or a Graphify / RAG / evidence summary.
- Preserve `$zl-cockpit-build` in user-facing notes.

## Required Flow

1. Resolve the project root from the current working directory.
2. Run the local Zhulong cockpit builder:
...
- GraphRAG index, Graphify build, and refresh-run require explicit user approval through `--run`, `--index`, or `zl-refresh-run`.
