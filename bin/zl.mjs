...
function usage() {
  console.log(`Zhulong Project Intelligence Kit
...
function ragInitLocal(args) {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error("Missing .planning/config.json. Run zl-init first.");
  }
  ...
  if (!commandAvailable("graphrag")) {
    throw new Error("GraphRAG CLI is not installed. Install it before running zl-rag-init-local.");
  }
...
function appendEvidenceWriteback(target, recordPath, evidencePath, details) {
  ...
  const section = `\n\n## Zhulong Evidence Writeback - ${generatedAt}\n\n`
...
function runtimeStatusCommand(args) {
  ...
  for (const item of items) {
    ...
    let rendered = false;
    if (installed) {
      rendered = !fs.readFileSync(installedPath, "utf8").includes("{{ZL_CLI}}");
    }
...
async function main() {
  try {
    if (args.command === "help" && args._[0] === "skills") {
      ...
      answer(args);
    } else if (args.command === "ambiguity") {
      ambiguity(args);
    } else if (args.command === "structure") {
      structure(args);
