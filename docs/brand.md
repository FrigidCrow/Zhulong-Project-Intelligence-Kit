...
| 品牌目的 | 照亮 AI 编程看不见的项目状态、规格依据、影响面和完成条件。 |
| 愿景 | 让 AI 参与的软件工程具备可解释、可复跑、可审计的项目记忆。 |
| 使命 | 把本地文档、代码地图、工作流门禁和验证证据连接成一套确定性的项目智能协议。 |
...
对于文档密集、既有代码复杂、交付必须说明依据的软件团队，Zhulong 是一套本地项目智能工具。它在 AI 修改代码前提供项目记忆，在修改过程中提供影响和策略边界，在任务完成前提供机械门禁。它不同于普通聊天助手，因为关键判断会落成可检查的本地制品，而不是停留在对话里。

### 核心受众

- 需要处理规格书、QA、会议纪要和旧系统的开发者。
...
- 用户问题：AI 的规格结论、测试结果和完成声明容易停留在聊天里。
