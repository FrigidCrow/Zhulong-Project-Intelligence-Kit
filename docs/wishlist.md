...
Zhulong 是一个同时适配文档密集型与非文档密集型软件开发的 **通用 AI 项目智能框架**。
...
> 让 AI 不只是会写代码，而是在项目状态、代码影响面、按需文档依据和验证闭环里写代码。

## 2. 目标场景

Zhulong 不绑定国家、语言或行业。文档密集型项目可以接入：
...
非文档密集型项目使用 `reference + rag none`，无需安装或建立 RAG 索引，workflow、代码地图、策略、evidence 和完成门禁仍完整可用。项目始终以开发为主，文档/RAG 是按需上下文层，不是独立知识库，也不是使用前提。
