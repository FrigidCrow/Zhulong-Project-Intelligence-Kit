# Zhulong Developer Audit

Zhulong Developer Audit 是维护者专用的常态审计和对标机制。它不面向普通使用者，不新增公开 `zl-*` 命令，也不会进入 `docs/commands.html`。

目标是回答四个问题：

- 当前 74 个 `zl` / `zl-*` 公开逻辑命令是否仍然可执行、可文档化、可追溯。
- Codex / Claude Code / GitHub Copilot 的 33 个 runtime skill/prompt 是否真的能安装并引导到本地 `zl-*`。
...
- Zhulong 与 GSD / Superpowers 做同题对标时，差异在哪里，耗时和 token 怎么记录。
...
| 命令 | 用途 |
| --- | --- |
| `npm run verify:dev-audit-harness` | 验证 `.zl-audit/` 已被 git ignore、npm 脚本存在、fixture 能生成。 |
...
| `npm run dev:audit:security-governance` | 生成 `.zl-audit/latest/SECURITY_GOVERNANCE_CHECK.*`，复查 local-only 和外部 RAG opt-in 边界。 |
| `npm run dev:audit:ragas-style` | 生成 `.zl-audit/latest/RAGAS_STYLE_KNOWLEDGE_SCORES.*`，本地代理 Ragas 的 context recall / faithfulness / citation / tool call / goal 指标。 |
| `npm run dev:audit:promptfoo-redteam` | 生成 `.zl-audit/latest/PROMPTFOO_STYLE_REDTEAM_SCORES.*`，本地代理 Promptfoo 红队矩阵。 |
| `npm run dev:audit:benchmark` | 只跑 Zhulong / GSD / Superpowers 对标。 |
| `npm run dev:audit:nightly` | 维护者夜间巡检入口，默认不跑真实 AI 子进程。 |

## 产物位置

原始产物全部写入 `.zl-audit/`，并且 `.gitignore` 已经排除：

```text
.zl-audit/runs/<timestamp>/
.zl-audit/latest/
```
...
.zl-audit/latest/AUDIT_SCORECARD.json
.zl-audit/latest/AUDIT_SCORECARD.html
...
.zl-audit/latest/QUALITY_CONTROL_SCORECARD.html
...
`SKILL_SCORES` 只证明 runtime skill/prompt 的结构合规；`SKILL_BEHAVIOR_SCORES` 证明渲染后的 Codex / Claude Code / GitHub Copilot runtime pack 是否包含正确 `zl-*` 路由、local-only/privacy、no hidden heavy refresh、evidence/writeback 和 adversarial external-RAG guard。它按每个 runtime skill/prompt 五类 case 归档，是 deterministic harness，不调用外部模型。
...
| 场景 | 文档状态 | 期待 |
| --- | --- | --- |
| `docs-complete` | 仕様、QA、議事録 都存在 | Zhulong full-local 和 graph-lite 都应通过。 |
...
| 工具 | 本轮处理方式 |
| --- | --- |
| Zhulong `graph-lite-dev-loop` | 使用 Zhulong + Graphify，不跑 GraphRAG/RAG。 |
| Zhulong `full-local-graphify-graphrag` | 使用 Zhulong + Graphify + 本地 RAG/GraphRAG fixture。 |
...
GSD / Superpowers 没有本仓库内的 `zl-*` 等价 CLI，因此报告会记录 `instruction-pack.json`、skill hash、指令摘录、fixture、代码改修、测试和证据文件，并给 replay 分数设置可信度上限。它不会假装它们拥有 Zhulong 的 repository-local CLI、local GraphRAG default mode 或完整 `.planning/` evidence/policy 闭环。
...
```bash
ZHULONG_AUDIT_REAL_AI=1 npm run dev:audit:benchmark
```
...
```bash
ZHULONG_AUDIT_CODEX_IGNORE_USER_CONFIG=1 ZHULONG_AUDIT_REAL_AI=1 npm run dev:audit:benchmark
```
...
- `.zl-audit/latest/BENCHMARK_COMPARISON.md` 必须显示 `Memory isolation: PASS`。
...
`AUDIT_SCORECARD.json` 里的 `Benchmark comparison: 87` 是全部 benchmark 行的平均分，不是 Zhulong 单体分。本轮平均了 12 行结果：

- Zhulong `graph-lite-dev-loop`
- Zhulong `full-local-graphify-graphrag`
- GSD `skill-pack-backed-replay`
- Superpowers `skill-pack-backed-replay`
- 三种文档状态：`docs-complete`、`docs-missing`、`docs-partial`

它偏保守有三个原因：`graph-lite` 是低成本模式，不按 full-local intelligence 给满分；`full-local` 在无文档场景输出 `EXPECTED_BLOCK`，这是正确阻断但会拉低平均；GSD / Superpowers 不是本仓库内可执行 CLI，也不是 live model benchmark，所以设置可信度上限。看 Zhulong 产品本身，应看三方总览里的 `Zhulong: 90 / A`。
...
```bash
ZHULONG_AUDIT_REAL_AI=1 ZHULONG_AUDIT_CODEX_IGNORE_USER_CONFIG=1 ZHULONG_AUDIT_CODEX_TIMEOUT_MS=60000 npm run dev:audit:full
```
...
结果：Zhulong / GSD / Superpowers 三个 real-codex subprocess 都在启动阶段失败，fixture 代码未被修改。原因是当前 Codex CLI 默认模型 `gpt-5.3-codex` 对当前 ChatGPT account 不支持。`codex-events.jsonl` 中的关键错误为：

```text
The 'gpt-5.3-codex' model is not supported when using Codex with a ChatGPT account.
```

因此真实 AI token usage 本轮仍为 `TOKEN_USAGE_UNAVAILABLE`。这不是未执行，而是已执行且被当前账号/模型配置阻断。后续如果切换到可用 Codex 模型或 profile，再重跑 `ZHULONG_AUDIT_REAL_AI=1 npm run dev:audit:full`，`TOKEN_USAGE.md/json` 会继续尝试从 JSONL usage events 中提取 token。

结论：

- Zhulong 当前优势是 repository-local `zl-*` CLI、runtime skills、`.planning/` evidence、Graphify/RAG 耗时拆分和 local-only guard 能在同一 fixture 中落地。
- Zhulong 当前弱点是 full-local 比 graph-lite 更重，命令面也更大；日常开发必须继续靠 profile/policy 控制何时刷新 Graphify/RAG。
- GSD 的优势是 workflow 设计成熟，plan/execute/verify、UAT、subagent 编排和文档流程经验很强；短板是它不是当前项目的 repository-local Zhulong CLI，本地 RAG/Graphify/policy 默认融合不是原生闭环。
...
- 默认 deterministic benchmark 不统计 token；真实 token 只来自显式 `ZHULONG_AUDIT_REAL_AI=1` 的 Codex JSONL。
...
不要把 `dev:audit:*` 加到 `docs/commands.html`。它们是维护者 npm 脚本，不是用户使用的 `zl-*` 产品命令。
